...
export default function PlayerView({ roomCode, me, stage, roundCount, onFold, onToggleReveal, onLeave, onRefresh }: PlayerViewProps) {
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (scrollRef.current && scrollRef.current.scrollTop === 0) startY.current = e.touches[0].clientY;
  };
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (dy > 0 && scrollRef.current?.scrollTop === 0) setPullY(Math.min(dy * 0.4, 120)); 
  };
  ...
  const handleTouchEnd = async () => {
    if (pullY > 60) {
      ...
      setPullY(60); 
      await onRefresh();
      setTimeout(() => { setIsRefreshing(false); setPullY(0); startY.current = 0; }, 500);
    } else {
  ...
  return (
    <div 
      ref={scrollRef}
      className="min-h-[100.1vh] bg-neutral-900 flex flex-col relative text-white overflow-y-auto no-scrollbar"
    ...
    >
      {/* PULL INDICATOR */}
      <div 
        className="absolute top-0 left-0 w-full flex justify-center items-center pointer-events-none transition-all duration-200 ease-out z-50"
        style={{ height: `${pullY}px`, opacity: pullY > 0 ? 1 : 0 }}
      >
        <div className="bg-emerald-600/20 backdrop-blur-md rounded-full px-4 py-1 text-emerald-400 text-xs font-bold uppercase tracking-widest border border-emerald-500/50 shadow-xl flex items-center gap-2">
           {isRefreshing ? <span className="animate-spin">⟳</span> : <span className={`transform transition-transform ${pullY > 60 ? 'rotate-180' : ''}`}>↓</span>}
           {isRefreshing ? "SYNCING..." : pullY > 60 ? "RELEASE TO SYNC" : "PULL TO SYNC"}
        </div>
      </div>

      <div className="flex flex-col p-4 transition-transform duration-200 ease-out" style={{ transform: `translateY(${pullY}px)` }}>
        
        {/* HEADER */}
        <div className="flex justify-between items-center mb-2 shrink-0">
           <div className="flex flex-col">
             <div className="text-gray-400 text-[10px] font-mono">ROOM: {roomCode}</div>
             {/* --- NEW: ROUND BADGE --- */}
             <div className="text-purple-400 text-xs font-bold tracking-widest">ROUND {roundCount}</div>
           </div>
           
           <div className="flex gap-2">
             <button onClick={onRefresh} className="text-emerald-500 text-[10px] border border-emerald-900 px-2 py-1 rounded active:bg-emerald-900/20">SYNC</button>
            ...
            <>
              ...
              {isFolded && (
                 <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-red-600 text-5xl md:text-8xl font-black border-[8px] md:border-[12px] border-red-600 p-4 md:p-8 rounded-3xl -rotate-12 tracking-widest opacity-80 mix-blend-screen bg-black/40">FOLDED</div>
                 </div>
              )}
